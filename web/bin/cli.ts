...
switch (command) {
  ...
  case "install-adapter": {
    ...
    try {
      const installed = await registry.install(pkg);
      console.log(`Installed adapter: ${installed.metadata.displayName} (${installed.metadata.name})`);
      console.log(`  Version: ${installed.metadata.version}`);
      console.log(`  Protocol: ${installed.metadata.protocol}`);
      console.log(`  Models: ${installed.metadata.models.map((m) => m.label).join(", ")}`);
    } catch (err) {
      console.error(`Failed to install adapter: ${err instanceof Error ? err.message : err}`);
      process.exit(1);
    }
    break;
  }
  ...
  case "uninstall-adapter": {
    ...
    if (reg.uninstall(name)) {
      console.log(`Uninstalled adapter: ${name}`);
    } else {
      console.error(`Adapter "${name}" not found`);
      process.exit(1);
    }
    break;
  }
