...
export type BrowserOutgoingMessage =
  ...
  // ── Collective Intelligence: Browser → Server ───────────────────────────────
  | { type: "memory_query"; query: string; limit?: number }
  | { type: "memory_store"; content: string; memoryType: string; tags: string[]; gitContext?: Record<string, unknown> }
  | { type: "deliberation_respond"; proposalId: string; stance: string; reasoning: string; suggestedAlternative?: string; concerns?: string[] }
  ...
  | { type: "inject_thought"; content: string; thoughtType: string; parentId?: string };
...
export type BrowserIncomingMessageBase =
  ...
  | { type: "presence_update"; viewers: PresenceViewer[] }
  | { type: "role_assigned"; role: SessionRole; viewerId: string }
  | { type: "vote_update"; request_id: string; votes: PermissionVote[]; voters_total: number; deadline: number }
  | { type: "vote_resolved"; request_id: string; result: "allow" | "deny"; policy: VotingPolicy }
  // ── Collective Intelligence: Server → Browser ───────────────────────────────
  ...
  | { type: "deliberation_resolved"; resolution: import("./deliberation-engine.js").DeliberationResolution }
  // Layer 3: Capabilities & Routing
...
export type BackendType = "claude" | "codex" | "goose" | "aider" | "openhands" | "openclaw" | "opencode";
...
export interface SessionState {
  ...
  // Claude-specific token details (extracted from result.modelUsage at turn end)
...
/** Roles for shared session access control */
export type SessionRole = "owner" | "collaborator" | "spectator";

/** Viewer info for presence tracking */
...
/** Policy for how votes from collaborators are aggregated */
export type VotingPolicy = "majority-rules" | "any-deny-blocks" | "owner-decides";

/** Individual vote cast by a collaborator */
