...
describe("Session management", () => {
  ...
  it("getOrCreateSession: does NOT overwrite backendType when called without explicit type", () => {
    // Simulate: attachAdapter creates session as "codex"
    const session = bridge.getOrCreateSession("s1", "codex");
    expect(session.backendType).toBe("codex");
    expect(session.state.backend_type).toBe("codex");
  ...
  it("getOrCreateSession: overwrites backendType when explicitly provided on existing session", () => {
    ...
    // Explicit override (e.g. attachAdapter)
...
describe("Browser handlers", () => {
  ...
  it("handleBrowserOpen: does NOT relaunch when Codex adapter is attached but still initializing", () => {
    ...
    const session = bridge.getOrCreateSession("s1", "codex");
    session.adapter = { isConnected: () => false } as any;
  ...
  it("session_subscribe: replays buffered sequenced events after last_seq", () => {
    ...
    // Events after seq=1: 2 stream_events + 1 presence_update from handleBrowserOpen
...
describe("WsBridge invite tokens", () => {
  let bridge: WsBridge;
  ...
  it("createInviteToken returns null for unknown session", () => {
    expect(bridge.createInviteToken("nonexistent")).toBeNull();
  });

  it("createInviteToken returns a non-empty token string for a valid session", () => {
    bridge.getOrCreateSession("s1");
    const token = bridge.createInviteToken("s1");
    expect(token).not.toBeNull();
    expect(typeof token).toBe("string");
    expect(token!.length).toBeGreaterThan(0);
  });

  it("createInviteToken stores the role associated with the token", () => {
    // Note: randomUUID is mocked to a fixed value, so repeated calls produce the same token.
    // In production, randomUUID returns unique values each time.
    bridge.getOrCreateSession("s1");
    const token = bridge.createInviteToken("s1", "spectator");
    expect(token).not.toBeNull();
    // The token should resolve to the correct role
    expect(bridge.resolveInviteTokenRole(token!)).toBe("spectator");
  });

  it("resolveInviteToken returns the correct sessionId", () => {
    bridge.getOrCreateSession("s1");
    const token = bridge.createInviteToken("s1")!;
    expect(bridge.resolveInviteToken(token)).toBe("s1");
  });

  it("resolveInviteToken returns null for an invalid token", () => {
    expect(bridge.resolveInviteToken("badtoken1234")).toBeNull();
  });

  it("resolveInviteTokenRole returns the correct role", () => {
    bridge.getOrCreateSession("s1");
    const token = bridge.createInviteToken("s1", "spectator")!;
    expect(bridge.resolveInviteTokenRole(token)).toBe("spectator");
  });

  it("resolveInviteTokenRole defaults to collaborator", () => {
    bridge.getOrCreateSession("s1");
    const token = bridge.createInviteToken("s1")!;
    expect(bridge.resolveInviteTokenRole(token)).toBe("collaborator");
  });

  it("resolveInviteTokenRole returns null for invalid token", () => {
    expect(bridge.resolveInviteTokenRole("badtoken")).toBeNull();
  });
});

// ── Presence tests ──────────────────────────────────────────────────────────

describe("WsBridge presence tracking", () => {
  let bridge: WsBridge;

  function makeBrowser(sessionId: string) {
    return {
      data: { kind: "browser", sessionId } as any,
      send: vi.fn(),
      close: vi.fn(),
      readyState: 1,
    } as any;
  ...
  it("assigns owner role to first browser", () => {
    bridge.getOrCreateSession("s1");
    const browser = makeBrowser("s1");
    bridge.handleBrowserOpen(browser, "s1");

    const data = browser.data as any;
    expect(data.role).toBe("owner");
    expect(data.viewerId).toBeDefined();
    expect(data.viewerName).toBeDefined();
  });

  it("assigns collaborator role to subsequent browsers (no invite token)", () => {
    bridge.getOrCreateSession("s1");
    const browser1 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser1, "s1");

    const browser2 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser2, "s1");

    expect((browser1.data as any).role).toBe("owner");
    expect((browser2.data as any).role).toBe("collaborator");
  });

  it("uses _joinRole from invite token for subsequent browsers", () => {
    bridge.getOrCreateSession("s1");
    const browser1 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser1, "s1");

    const browser2 = makeBrowser("s1");
    (browser2.data as any)._joinRole = "spectator";
    bridge.handleBrowserOpen(browser2, "s1");

    expect((browser2.data as any).role).toBe("spectator");
  });

  it("getSessionViewers returns all connected viewers", () => {
    bridge.getOrCreateSession("s1");
    const browser1 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser1, "s1");
    const browser2 = makeBrowser("s1");
    ...
    const viewers = bridge.getSessionViewers("s1");
    expect(viewers).toHaveLength(2);
    expect(viewers[0].role).toBe("owner");
    expect(viewers[1].role).toBe("collaborator");
  });

  it("broadcasts presence_update on connect and disconnect", () => {
    bridge.getOrCreateSession("s1");
    const browser1 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser1, "s1");

    // First browser should receive presence_update with itself
    const calls1 = (browser1.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const presenceMsg = calls1.find((c: any) => c.type === "presence_update");
    expect(presenceMsg).toBeDefined();
    expect(presenceMsg.viewers).toHaveLength(1);

    // Connect second browser
    const browser2 = makeBrowser("s1");
    bridge.handleBrowserOpen(browser2, "s1");

    // Both should have received presence_update with 2 viewers
    const calls2 = (browser2.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const presenceMsg2 = calls2.find((c: any) => c.type === "presence_update" && c.viewers?.length === 2);
    expect(presenceMsg2).toBeDefined();

    // Disconnect browser2 — browser1 should get updated presence
    (browser1.send as any).mockClear();
    bridge.handleBrowserClose(browser2);

    const calls3 = (browser1.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const presenceAfterDisconnect = calls3.find((c: any) => c.type === "presence_update");
    expect(presenceAfterDisconnect).toBeDefined();
    expect(presenceAfterDisconnect.viewers).toHaveLength(1);
  });

  it("sends role_assigned message on browser connect", () => {
    bridge.getOrCreateSession("s1");
    const browser = makeBrowser("s1");
    bridge.handleBrowserOpen(browser, "s1");

    const calls = (browser.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const roleMsg = calls.find((c: any) => c.type === "role_assigned");
    expect(roleMsg).toBeDefined();
    expect(roleMsg.role).toBe("owner");
    expect(roleMsg.viewerId).toBeDefined();
  });
});

// ── RBAC enforcement tests ──────────────────────────────────────────────────

describe("WsBridge RBAC enforcement", () => {
  let bridge: WsBridge;

  function makeBrowser(sessionId: string, role?: string) {
    const ws = {
      data: { kind: "browser", sessionId, role } as any,
      send: vi.fn(),
      close: vi.fn(),
      readyState: 1,
    } as any;
  ...
  function makeCliSocket(sessionId: string) {
    return {
      data: { kind: "cli", sessionId } as any,
      send: vi.fn(),
      close: vi.fn(),
      readyState: 1,
    } as any;
  ...
  it("blocks spectator from sending user_message", () => {
    const cli = makeCliSocket("s1");
    bridge.handleCLIOpen(cli, "s1");

    const spectator = makeBrowser("s1", "spectator");
    bridge.getSession("s1")!.browserSockets.add(spectator);

    bridge.handleBrowserMessage(spectator, JSON.stringify({
      type: "user_message",
      content: "hello",
    }));

    // CLI should NOT have received the message
    const cliCalls = (cli.send as any).mock.calls;
    const userMsgs = cliCalls.filter(([arg]: [string]) => arg.includes('"type":"user"'));
    expect(userMsgs).toHaveLength(0);

    // Spectator should get an error message
    const errorCalls = (spectator.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
    const errorMsg = errorCalls.find((c: any) => c.type === "error");
    expect(errorMsg).toBeDefined();
    expect(errorMsg.message).toContain("Spectators");
  });

  it("blocks spectator from sending permission_response", () => {
    const cli = makeCliSocket("s1");
    bridge.handleCLIOpen(cli, "s1");
    ...
    // CLI should NOT have received the response
    const cliCalls = (cli.send as any).mock.calls;
    const controlResponses = cliCalls.filter(([arg]: [string]) => arg.includes('"control_response"'));
    expect(controlResponses).toHaveLength(0);
  });

  it("allows collaborator to send user_message", () => {
    const cli = makeCliSocket("s1");
    bridge.handleCLIOpen(cli, "s1");

    const collaborator = makeBrowser("s1", "collaborator");
    bridge.getSession("s1")!.browserSockets.add(collaborator);

    bridge.handleBrowserMessage(collaborator, JSON.stringify({
      type: "user_message",
      content: "hello from collaborator",
    }));

    // CLI should have received the message
    const cliCalls = (cli.send as any).mock.calls;
    const userMsgs = cliCalls.filter(([arg]: [string]) => arg.includes('"type":"user"'));
    expect(userMsgs).toHaveLength(1);
  });

  it("allows spectator to subscribe and ack", () => {
    bridge.getOrCreateSession("s1");
    const spectator = makeBrowser("s1", "spectator");
    bridge.getSession("s1")!.browserSockets.add(spectator);

    // session_subscribe and session_ack should not be blocked
    bridge.handleBrowserMessage(spectator, JSON.stringify({
      type: "session_subscribe",
      last_seq: 0,
    }));
    ...
    const calls = (spectator.send as any).mock.calls.map(([arg]: [string]) => JSON.parse(arg));
...
describe("WsBridge voting policy", () => {
  let bridge: WsBridge;
  ...
  it("defaults to majority-rules", () => {
    expect(bridge.getVotingPolicy()).toBe("majority-rules");
  });

  it("can set and get voting policy", () => {
    bridge.setVotingPolicy("any-deny-blocks");
    expect(bridge.getVotingPolicy()).toBe("any-deny-blocks");
    bridge.setVotingPolicy("owner-decides");
