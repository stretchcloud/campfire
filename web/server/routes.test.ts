...
function createMockBridge() {
  return {
    ...
    createInviteToken: vi.fn(() => "abc123def456"),
    resolveInviteToken: vi.fn(() => null),
...
describe("GET /api/backends", () => {
  it("returns all backends with availability status", async () => {
    // resolveBinary returns a path for all binaries (claude, codex, goose, aider, openhands, openclaw, opencode)
    mockResolveBinary
      .mockReturnValueOnce("/usr/bin/claude")
      .mockReturnValueOnce("/usr/bin/codex")
      .mockReturnValueOnce("/usr/bin/goose")
      .mockReturnValueOnce("/usr/bin/aider")
      .mockReturnValueOnce("/usr/bin/openhands")
      .mockReturnValueOnce("/usr/bin/openclaw")
    ...
    expect(json).toEqual([
      ...
      { id: "goose", name: "Goose", available: true },
      { id: "aider", name: "Aider", available: true },
      { id: "openhands", name: "OpenHands", available: true },
      { id: "openclaw", name: "OpenClaw", available: true },
      { id: "opencode", name: "OpenCode", available: true },
    ]);
  });

  it("marks backends as unavailable when binary is not found", async () => {
    // resolveBinary returns null for all (claude, codex, goose, aider, openhands, openclaw, opencode)
    mockResolveBinary
    ...
    expect(json).toEqual([
      ...
      { id: "goose", name: "Goose", available: false },
      { id: "aider", name: "Aider", available: false },
      { id: "openhands", name: "OpenHands", available: false },
      { id: "openclaw", name: "OpenClaw", available: false },
  ...
  it("handles mixed availability", async () => {
    mockResolveBinary
      ...
      .mockReturnValueOnce("/usr/bin/goose") // goose found
      .mockReturnValueOnce(null) // aider not found
      .mockReturnValueOnce(null) // openhands not found
      .mockReturnValueOnce(null) // openclaw not found
      .mockReturnValueOnce(null); // opencode not found
...
describe("POST /api/sessions/:id/invite", () => {
  it("returns a token and URL for a valid session", async () => {
    const res = await app.request("/api/sessions/session-1/invite", { method: "POST" });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.token).toBe("abc123def456");
    expect(json.url).toContain("/#/join/abc123def456");
    expect(bridge.createInviteToken).toHaveBeenCalledWith("session-1", "collaborator");
  });

  it("returns 404 when session does not exist", async () => {
    bridge.createInviteToken.mockReturnValue(null);
    const res = await app.request("/api/sessions/nonexistent/invite", { method: "POST" });
    expect(res.status).toBe(404);
    const json = await res.json();
...
describe("GET /api/sessions/join/:token", () => {
  it("resolves a valid token to a session ID", async () => {
    bridge.resolveInviteToken.mockReturnValue("session-1");
    const res = await app.request("/api/sessions/join/abc123def456", { method: "GET" });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.session_id).toBe("session-1");
    expect(bridge.resolveInviteToken).toHaveBeenCalledWith("abc123def456");
  });

  it("returns 404 for an invalid token", async () => {
    bridge.resolveInviteToken.mockReturnValue(null);
    const res = await app.request("/api/sessions/join/badtoken1234", { method: "GET" });
    expect(res.status).toBe(404);
    const json = await res.json();
    expect(json.error).toBe("Invalid or expired invite token");
  });
});
