...
${MARKER_END}`;
  ...
  /**
   * Detect running Claude Code CLI processes that could be adopted.
   * Scans for `claude` processes with `--sdk-url` arguments.
   */
  async detectRunningProcesses(): Promise<DetectedProcess[]> {
    try {
      ...
      for (const info of this.sessions.values()) {
        if (info.pid) knownPids.add(info.pid);
      }

      for (const line of output.split("\n")) {
        // Match claude processes with --sdk-url
        ...
        // Skip processes already managed by us
        if (knownPids.has(pid)) continue;

        // Extract cwd from /proc if available
        let cwd = process.cwd();
        try {
          cwd = await Bun.file(`/proc/${pid}/cwd`).text().catch(() => "");
          if (!cwd) {
            // Try readlink for /proc/pid/cwd
            const lsProc = Bun.spawn(["readlink", `/proc/${pid}/cwd`], { stdout: "pipe" });
            cwd = (await new Response(lsProc.stdout).text()).trim() || process.cwd();
            await lsProc.exited;
          }
        } catch {
          cwd = process.cwd();
        ...
        // Extract --sdk-url value
        const cmdline = parts.slice(10).join(" ");
        const sdkUrlMatch = cmdline.match(/--sdk-url\s+(\S+)/);
        const sdkUrl = sdkUrlMatch ? sdkUrlMatch[1] : undefined;

        // Extract --model value
        const modelMatch = cmdline.match(/--model\s+(\S+)/);
        const model = modelMatch ? modelMatch[1] : undefined;

        // Extract --resume value (Claude CLI session ID)
        const resumeMatch = cmdline.match(/--resume\s+(\S+)/);
        const cliSessionId = resumeMatch ? resumeMatch[1] : undefined;

        detected.push({ pid, cwd, sdkUrl, model, cliSessionId, cmdline });
      }

      return detected;
    } catch (e) {
      console.error("[cli-launcher] Failed to detect running processes:", e);
      return [];
    }
  }

  /**
   * Adopt an external Claude Code CLI process by creating a session entry
   * and relaunching it with --resume pointing at this server's WebSocket.
   */
  async adopt(options: AdoptOptions): Promise<SdkSessionInfo> {
    const sessionId = randomUUID();
    const cwd = options.cwd || process.cwd();
    ...
    // Kill the old process gracefully so we can relaunch with --resume
    if (options.pid) {
      try {
    ...
    // Relaunch with --resume pointing at our WebSocket
    if (options.cliSessionId) {
      this.spawnCLI(sessionId, info, {
        model: options.model,
        cwd,
        resumeSessionId: options.cliSessionId,
      });
    } else {
      // No cliSessionId — just spawn fresh
      this.spawnCLI(sessionId, info, {
        model: options.model,
        cwd,
      });
...
/** A running Claude Code process detected on the system. */
...
/** Options for adopting an external CLI process. */
export interface AdoptOptions {
  /** PID of the process to adopt (will be killed and relaunched). */
  pid?: number;
  /** Working directory for the session. */
  cwd?: string;
  /** Model the process was using. */
  model?: string;
  /** Claude CLI's internal session ID (for --resume). */
  cliSessionId?: string;
}
