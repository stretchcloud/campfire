...
describe("getModelsForBackend", () => {
  ...
  it("returns goose models for goose backend", () => {
    expect(getModelsForBackend("goose")).toBe(GOOSE_MODELS);
  });

  it("returns openclaw models for openclaw backend", () => {
    expect(getModelsForBackend("openclaw")).toBe(OPENCLAW_MODELS);
  });
});
...
describe("getModesForBackend", () => {
  ...
  it("returns goose modes for goose backend", () => {
    expect(getModesForBackend("goose")).toBe(GOOSE_MODES);
  });

  it("returns openclaw modes for openclaw backend", () => {
    expect(getModesForBackend("openclaw")).toBe(OPENCLAW_MODES);
  });
});
...
describe("getDefaultModel", () => {
  ...
  it("returns first goose model for goose backend", () => {
    expect(getDefaultModel("goose")).toBe(GOOSE_MODELS[0].value);
  });

  it("returns first openclaw model for openclaw backend", () => {
    expect(getDefaultModel("openclaw")).toBe(OPENCLAW_MODELS[0].value);
  });
});
...
describe("getDefaultMode", () => {
  ...
  it("returns first goose mode for goose backend", () => {
    expect(getDefaultMode("goose")).toBe(GOOSE_MODES[0].value);
  });

  it("returns first openclaw mode for openclaw backend", () => {
    expect(getDefaultMode("openclaw")).toBe(OPENCLAW_MODES[0].value);
  });
});
...
describe("static model/mode lists", () => {
  ...
  it("has goose models including default", () => {
    expect(GOOSE_MODELS.length).toBeGreaterThanOrEqual(1);
    expect(GOOSE_MODELS[0].value).toBe("default");
  });

  it("has openclaw models including default", () => {
    expect(OPENCLAW_MODELS.length).toBeGreaterThanOrEqual(1);
    expect(OPENCLAW_MODELS[0].value).toBe("default");
  });
  ...
  it("has at least 2 modes for each backend", () => {
    ...
    expect(GOOSE_MODES.length).toBeGreaterThanOrEqual(2);
    expect(OPENCLAW_MODES.length).toBeGreaterThanOrEqual(2);
  });
});
