...
export function MessageBubble({ message, onFork }: { message: ChatMessage; onFork?: () => void }) {
  ...
  if (message.role === "user") {
    return (
      <div className="group/msg flex justify-end animate-[fadeSlideIn_0.2s_ease-out] relative">
        {onFork && (
          <button
            onClick={onFork}
            className="absolute left-0 top-1 opacity-0 group-hover/msg:opacity-100 transition-opacity p-1 rounded hover:bg-cc-hover text-cc-muted hover:text-cc-fg"
  ...
  // Assistant message
  return (
    <div className="group/msg animate-[fadeSlideIn_0.2s_ease-out] relative">
      {onFork && (
        <button
          onClick={onFork}
          className="absolute right-0 top-1 opacity-0 group-hover/msg:opacity-100 transition-opacity p-1 rounded hover:bg-cc-hover text-cc-muted hover:text-cc-fg z-10"
          title="Fork session from here"
        >
          <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5">
            <path fillRule="evenodd" d="M5 3.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm0 2.122a2.25 2.25 0 10-1.5 0v.878A2.25 2.25 0 005.75 8.5h1.5v2.128a2.251 2.251 0 101.5 0V8.5h1.5a2.25 2.25 0 002.25-2.25v-.878a2.25 2.25 0 10-1.5 0v.878a.75.75 0 01-.75.75h-4.5A.75.75 0 015 6.25v-.878zm3.75 7.378a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm3-8.75a.75.75 0 100-1.5.75.75 0 000 1.5z" />
