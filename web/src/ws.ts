...
function handleParsedMessage(
  ...
  switch (data.type) {
    case "session_init": {
      ...
      // Record session start time (only once, don't overwrite on reconnect)
      if (!useStore.getState().sessionStartTimes.has(sessionId)) {
        const times = new Map(useStore.getState().sessionStartTimes);
        times.set(sessionId, Date.now());
        useStore.setState({ sessionStartTimes: times });
      }
    ...
    case "role_assigned": {
      store.setMyRole(sessionId, data.role);
      store.setMyViewerId(sessionId, data.viewerId);
      break;
    }

    case "vote_update": {
      store.setPermissionVotes(sessionId, data.request_id, {
        votes: data.votes,
        votersTotal: data.voters_total,
    ...
    case "vote_resolved": {
      store.setVoteResult(sessionId, data.request_id, {
        result: data.result,
        policy: data.policy,
      });
      // Clear vote state after a short delay for UI to show result
      setTimeout(() => {
        store.clearVoteState(sessionId, data.request_id);
      }, 3000);
      break;
    }
